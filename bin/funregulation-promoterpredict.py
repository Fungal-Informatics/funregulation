...
args.genome = '/Users/arlenz/NetBeansProjects/funregulation/input/P.oxalicum114-2.fasta'
args.annotation = '/Users/arlenz/NetBeansProjects/funregulation/input/P.oxalicum114-2.gff'
            ...
            if record.ltype == 'gene':
                ...
                if record.strand == '+':
                    if record.start+upstream > 0:
                        outfp.write(gene+'\t'+record.strand+'\t'+record.seqid+'\t'+str(record.start+upstream)+'\t'+str(record.start+downstream+2)+'\n')
                    else:    
                        # incomplete promoters
                        outfp.write(gene+'\t'+record.strand+'\t'+record.seqid+'\t'+str(1)+'\t'+str(record.start+downstream+2)+'\n')
                        lib.log.info("Promoter from gene " + gene + " can't be fully extracted")
                        promoters_partially_extracted += 1
                else:
                    if record.end+upstream > 0:
                        outfp.write(gene+'\t'+record.strand+'\t'+record.seqid+'\t'+str(record.end+upstream)+'\t'+str(record.end+downstream-2)+'\n')
                    else:    
                        # incomplete promoters
                        outfp.write(gene+'\t'+record.strand+'\t'+record.seqid+'\t'+str(1)+'\t'+str(record.end+downstream-2)+'\n')
                        lib.log.info("Promoter from gene " + gene + " can't be fully extracted")
                        promoters_partially_extracted += 1
                recordCount += 1
...
def extract_promoters_from_genome_fasta_file(file_promoters_bed):
    ...
    lib.log.info("Promoters in"+args.out+'predict_misc')
