...
parser = argparse.ArgumentParser(prog='funregulation-promoterextract.py', usage="%(prog)s [options] -g genome.fasta -a annotation.gff3 -o /output_folder",
    description = '''Script that does it all.''',
    epilog = "Written by Alexandre Lenz (2017-2021) arlenz@ucs.br / alenz@uneb.br",
...
args.genome = '/Users/arlenz/NetBeansProjects/funregulation/input/Penicillium_sp_UCS_S1M29.fsa'
args.annotation = '/Users/arlenz/NetBeansProjects/funregulation/input/Penicillium_sp_UCS_S1M29.gff3'

#create folder structure
if not os.path.isdir(args.out):
    os.makedirs(args.out)
    os.makedirs(os.path.join(args.out, 'extract_misc'))
    os.makedirs(os.path.join(args.out, 'extract_results'))
    os.makedirs(os.path.join(args.out, 'logfiles'))
else:
    if os.path.isdir(os.path.join(args.out, 'extract_results')):
        shutil.rmtree(os.path.join(args.out, 'extract_results'))
        os.makedirs(os.path.join(args.out, 'extract_results'))
    #make sure subdirectories exist
    dirs = [os.path.join(args.out, 'extract_misc'), os.path.join(args.out, 'extract_results'), os.path.join(args.out, 'logfiles')]
    for d in dirs:
        if not os.path.isdir(d):
            os.makedirs(d)
        
#create log file
log_name = os.path.join(args.out, 'logfiles', 'funregulation-promoterextract.log')
...
def create_promoters_bed_file(file_promoters_bed):
    lib.log.info("Start promoter extraction")
    recordCount = 0
    ...
    if os.path.isfile(file_promoters_bed):
        ...
        lib.log.info("No such file. New promoter extraction file will be created")
    ...
    lib.log.info("Promoter extraction successful")
...
"""
Reads promoter's data (genes, scaffolds and postions) from bed file 
and extract each promoter to new fasta file
...
def extract_promoters_from_genome_fasta_file(file_promoters_bed):
    ...
    for gene in parse_bed(file_promoters_bed):
        ...
        with open(args.out+'extract_misc/'+gene.gene+'_promoter.fasta', 'w') as f:
            SeqIO.write(short_seq_record, f, 'fasta')
    lib.log.info("Promoters partially extracted: %d" % promoters_partially_extracted)
    lib.log.info("Promoters extraction succesful")
    lib.log.info("Promoters in"+args.out+'extract_misc')
    lib.log.info("funregulation Promoter Prediction Successful")
    sys.exit(0)
    

# file definitions
file_promoters_bed = os.path.join(args.out,'extract_results','promoters.bed')
